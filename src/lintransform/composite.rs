...
///
/// Computes the map that moves the coefficients w.r.t. the powerful basis into the slots.
/// Discards any component perpendicular to the `Fp`-span of the powerful basis vectors.
/// To achieve this, the returned trace must also be applied after the sequence of linear transforms.
/// 
/// See also [`odd_slots_to_powcoeffs_thin()`] for details on the powerful basis.
/// 
pub fn powcoeffs_to_slots_thin<R, F, A>(H: &HypercubeIsomorphism<R, F, A>) -> (Vec<LinearTransform<R, F, A>>, Trace)
    where R: RingStore,
        R::Type: StdZn,
        F: CyclotomicRingDecomposition<R::Type> + RingDecompositionSelfIso<R::Type>,
        A: Allocator + Clone,
